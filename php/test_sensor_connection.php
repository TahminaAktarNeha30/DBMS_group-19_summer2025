<?php
// Diagnostic tool for sensor data database connection and structure
header('Content-Type: application/json');
error_reporting(0);
ini_set('display_errors', 0);

try {
    echo json_encode(['step' => 'Starting diagnostic test...', 'status' => 'info']);
    echo "\n";
    ...
    if (!$tableCheck || $tableCheck->num_rows == 0) {
        echo json_encode(['step' => 'sensor_data table does not exist', 'status' => 'error']);
        echo "\n";
        ...
        echo json_encode(['step' => 'sensor_data table exists', 'status' => 'success']);
        echo "\n";
        ...
        // Check sample data
        $sampleData = $conn->query("SELECT * FROM sensor_data LIMIT 3");
        $samples = [];
    ...
    echo json_encode(['step' => 'Diagnostic test completed successfully', 'status' => 'success']);
