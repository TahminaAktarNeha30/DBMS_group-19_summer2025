<?php
// Diagnostic tool for market management database connection and structure
header('Content-Type: application/json');
error_reporting(0);
ini_set('display_errors', 0);

try {
    echo json_encode(['step' => 'Starting market diagnostic test...', 'status' => 'info']);
    echo "\n";
    ...
    if (!$tableCheck || $tableCheck->num_rows == 0) {
        echo json_encode(['step' => 'markets table does not exist', 'status' => 'error']);
        echo "\n";
        ...
        echo json_encode(['step' => 'markets table exists', 'status' => 'success']);
        echo "\n";
        ...
        // Check sample data
        $sampleData = $conn->query("SELECT * FROM markets LIMIT 3");
        $samples = [];
        ...
        // Check total count
        $countResult = $conn->query("SELECT COUNT(*) as total FROM markets");
        $totalCount = $countResult->fetch_assoc()['total'];
        echo json_encode(['step' => 'Total records', 'total' => $totalCount, 'status' => 'success']);
        echo "\n";
    }
    
    echo json_encode(['step' => 'Market diagnostic test completed successfully', 'status' => 'success']);
