document.addEventListener('DOMContentLoaded', function() {
    ...
    spoilageForm.addEventListener('submit', function(e) {
        ...
        const formData = {
            ...
            recyclingOutput: document.getElementById('recyclingOutput').value,
            recyclingQuantity: document.getElementById('recyclingQuantity').value,
        ...
        .then(data => {
            if (data.success) {
                ...
                newRow.innerHTML = `
                        ...
                        <td>${formData.sourceType}</td>
                        <td>${formData.quantityLost}</td>
                        <td>${formData.reason}</td>
                        <td>${formData.spoilageDate}</td>
                        <td>${formData.recyclingOutput}</td>
                        <td>${formData.recyclingQuantity}</td>
                        <td>${formData.financialLoss}</td>
                        <td>
            ...
            .then(data => {
                ...
                (data || []).forEach(rec => {
                    ...
                    row.innerHTML = `
                        ...
                        <td>${rec.recycling_output || ''}</td>
                        <td>${rec.recycling_quantity || ''}</td>
                        <td>${rec.financial_loss || ''}</td>
                        <td>
...
function editSpoilage(recordId) {
    fetch(`php/get_spoilage_record.php?id=${recordId}`)
        ...
        .then(rec => {
            ...
            document.getElementById('sourceType').value = rec.source_type || '';
            ...
            document.getElementById('recyclingOutput').value = rec.recycling_output || '';
            document.getElementById('recyclingQuantity').value = rec.recycling_quantity || '';
            document.getElementById('financialLoss').value = rec.financial_loss || '';
        })
        .catch(err => {
            console.error('Error:', err);
